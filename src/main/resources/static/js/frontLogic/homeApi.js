...
function laodNewProducts(){
  ...
  productList.forEach((p,i)=>{
    ...
    newProductDom.append(productHtml(p.image,p.name,p.price,p.category,p.description));
  });
}, 2000);
}
function laodTopPicks(){ 
  ...
  setTimeout(() => { 
    ...
    productList.forEach((p,i)=>{
      if(i%2==0)return true;
      topPicksDom.append(productHtml(p.image,p.name,p.price,p.category,p.description));
    })
  }, 2000);
}
...
function loadRecentlyVisited(){ 
  ...
  setTimeout(() => { 
    ...
    productList.forEach((p,i)=>{
      if(i%2==0)return true;
      RecentlyVisitDom.append(productHtml(p.image,p.name,p.price,p.category,p.description));
    })
  }, 2000);
}
...
app.controller('homeController',($scope,$compile,$http,$window,config)=>{
    ...
    if ($window.pageYOffset + $window.innerHeight >= document.body.offsetHeight - 100) {
      if ($scope.functionsToLoad.length > 0) {
        ...
        if (typeof nextFunction === 'function') {
          ...
          console.error("Error: The next function is not callable.", nextFunction);
        }
      }
    }
  });
  ...
  $scope.addToCart = (pid) =>{
    if($scope.user.id==0){
      ...
      wishlistCartInfoToaster("To unlock your Cart","please log in! Sign up or log in to save your favorites and continue shopping seamlessly!");
      return;
    }
    if(pid==0){
      return;
  ...
  $scope.addToWishlist=async (pid)=>{ 
    if($scope.user.id==0){ 
      // toastr.info("To unlock your Wishlist and Cart, please log in! Sign up or log in to save your favorites and continue shopping seamlessly!");
      wishlistCartInfoToaster("unlock your Wishlist","please log in! Sign up or log in to save your favorites and continue shopping seamlessly!");
      return;
    }
    if(pid==0){
      return;
                ...
                <h5 class="card-title">${title}</h5> 
                  ...
                  if(cat in $scope.subcatDict){ 
                   let d=$scope.subcatDict[cat];
                    h+=  `<div class="catNChild"><img src="/image/${d.image==""?img:d.image}"  class="subcatImg"/>
