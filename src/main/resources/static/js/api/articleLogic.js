...
function editArticle(btn) {
  ...
  OpenHide(
    "#articleFormDom, #articleBackBtn ",
    "#articleRowDom, #articleEditBtn"
  );
...
function onEditImg(img) {
  $(".imagefiles").prepend(
    `<textarea class="form-control" name="image" id="image" style="display:none" >${img}</textarea> `
  );
  $(".multipartFile").prepend(
    `<img class="col-md-2 img-thumbnail" id="myImage" src="${img}" alt="image" >`
  );
}
// Example usage:
      ...
      .then((base64Strings) => {
        base64Strings.forEach((img, i) => {
          ...
          if (i == 0) {
            ...
            h = `<textarea class="form-control" name="image${i + 1}" id="image${
              i + 1
            }" style="display:none" >${img}</textarea>`;
          }
          $(".imagefiles").prepend(h);
          $(".multipartFile").prepend(
            `<img class="col-md-2 img-thumbnail" id="myImage" src="${img}" alt="image${i}" >`
          );
