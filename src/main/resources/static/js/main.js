...
function checkPhoneNumber() {
  ...
  $("input[name='email']").on("input", function () {
    ...
    showMessage(
      this,
      emailPattern.test(email) ? "Email is valid." : "Invalid email format.",
      emailPattern.test(email) ? "success" : "error"
    );
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    function compressImage(img, file) {
      ...
      function tryCompressAgain() {
        canvas.toBlob(
          function (blob) {
            if (blob.size > 500000 && quality > 0.1) {
              // Compress further if still too large
              quality -= 0.1;
              tryCompressAgain();
            } else if (blob.size <= 500000) {
              console.log(
                `Compressed Image${processedImages} size: ${
                  blob.size / 1024
                } KB`
              );
              const reader = new FileReader();
